...
class ClearHarbor(object):
    ...
    def fetch_del_repos_name(self, pro_id):
        self.del_repos_name = []
        repos_res = self.client.session.get(self.repos_url, params={"project_id": pro_id,"detail":"true"})
        # TODO
        for repo in repos_res.json():
            if repo['pull_count'] == 0:
                del_repo_url = self.repos_url + "/" + repo['name']
                print(del_repo_url)
                self.client.session.delete(del_repo_url)
            if repo["tags_count"] > 5: 
