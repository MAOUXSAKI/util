...
def get_attr_list(worksheet, num=None):
    attr_list = []
    flag = False
    if num == None:
        num = 99
...
def set_sheet_data(worksheet, data_list, style=None, attr_list=None, num=None):
    ...
    if attr_list is None:
        attr_list = get_attr_list(worksheet, num)
    for i in range(len(data_list)):
        print(i)
        for j in range(len(attr_list)):
            if attr_list[j] == 'no':
                worksheet.cell(i + 2, j + 1, i + 1)
            else:
            ...
            if isinstance(style,list):
                ...
                style_data = style
            worksheet.cell(i + 2, j + 1).border = style_data.border
            worksheet.cell(i + 2, j + 1).font = style_data.font
            worksheet.cell(i + 2, j + 1).alignment = style_data.alignment
            worksheet.cell(i + 2, j + 1).number_format = style_data.number_format
